...
const Navbar = () => {
  ...
  // 👇 single line to tolerate different backend field names
  const isVerified = userData?.isVerified ?? userData?.verified ?? userData?.emailVerified ?? false;
  ...
  const sendVerificationOtp = async () => {
    // if already verified, do nothing (keeps your toast from popping)
          ...
          {open && (
            ...
            >
              <ul className="text-slate-800 text-sm">
                {/* Verify email item — only change is the small chip beside the title */}
                <li
                  role="menuitem"
                  tabIndex={0}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      {/* title + chip on one line */}
                      <div className="flex items-center gap-2">
                        <span className="font-medium">Verify email</span>
                        {isVerified && (
                          <span className="text-[10px] leading-none px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 pointer-events-none">
                            Verified
                          </span>
                        )}
                      </div>
